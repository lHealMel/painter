...
def painter_menuing():
    ...
    # button4 의 실행 문장
    def closing():
        question_box = tkinter.messagebox.askquestion(title = "종료 확인", message = "정말로 종료하시겠습니까?")
        if question_box == 'yes':
            menu_window.destroy()
        else:
            pass
    ...
    button3 = tkinter.Button(menu_window, text = "제작자", command = show_me, height = hei, width = wid,
                             anchor = "s").place(
        x = a, y = 200)
    button4 = tkinter.Button(menu_window, text = "종료", command = closing, height = hei, width = wid,
                             anchor = "s").place(
        x = a, y = 250)
